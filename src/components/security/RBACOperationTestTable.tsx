...
import { OperationTest, TestResult } from '@/types/rbac-tests';

interface RBACOperationTestTableProps {
  tests: OperationTest[];
  onRunTest: (test: OperationTest) => Promise<TestResult>;
  onRunAll: () => Promise<void>;
  isLoading: boolean;
}

export function RBACOperationTestTable({ 
  tests, 
  onRunTest, 
  onRunAll,
  isLoading 
}: RBACOperationTestTableProps) {
  ...
  const handleRunTest = async (test: OperationTest) => {
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Operation Access Tests</h3>
        <Button 
          onClick={onRunAll} 
          disabled={isLoading}
          size="sm"
          ...
          <TableBody>
            {tests.map((test) => (
              <TableRow key={test.id}>
                ...
                <TableCell>
                  <code className="text-xs bg-gray-100 px-2 py-1 rounded">{test.operation}</code>
                </TableCell>
                <TableCell>
