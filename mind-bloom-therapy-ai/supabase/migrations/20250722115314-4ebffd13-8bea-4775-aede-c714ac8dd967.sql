-- Create only the NEW tables for session analytics and monitoring
-- The session_analytics table already exists, so we skip it
...
USING (
  EXISTS (
    ...
    WHERE instant_sessions.id = session_analytics_events.session_id::uuid
...
USING (
  EXISTS (
    SELECT 1 FROM instant_sessions
    WHERE instant_sessions.id = session_quality_metrics.session_id::uuid
    AND (instant_sessions.therapist_id = auth.uid() OR 
         EXISTS (
           SELECT 1 FROM instant_session_participants
           WHERE instant_session_participants.session_id = instant_sessions.id
...
USING (
  EXISTS (
    SELECT 1 FROM instant_sessions
    WHERE instant_sessions.id = session_recordings.session_id::uuid
