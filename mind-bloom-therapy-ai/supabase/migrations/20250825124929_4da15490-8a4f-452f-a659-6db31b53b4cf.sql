-- Performance optimization for video conference queries (corrected)
...
-- Optimize session-related queries (using correct column names)
CREATE INDEX IF NOT EXISTS idx_instant_sessions_created_at ON instant_sessions(created_at DESC);
...
BEGIN
    ...
    SELECT
        ...
        'Added timestamp indexes'::text,
        'Faster session retrieval by date'::text,
