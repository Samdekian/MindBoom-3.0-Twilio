-- Fix database function security by handling dependencies properly
-- First drop dependent views, then functions, then recreate everything

-- Drop dependent views that use the functions we need to modify
...
DROP FUNCTION IF EXISTS public.get_user_email_safe(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.has_role(uuid, app_role) CASCADE;
DROP FUNCTION IF EXISTS public.is_admin(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.cleanup_old_performance_metrics() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_email_for_admin(uuid) CASCADE;
...
-- Recreate the user_roles_view that was dropped
...
SELECT 
  ...
  get_user_email_safe(p.id) as email,
  p.account_type,
  p.created_at,
  p.updated_at
FROM profiles p
